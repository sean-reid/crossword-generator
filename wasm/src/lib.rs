...
pub use solution::{Clue, CrosswordMetadata, CrosswordPuzzle, Placement};
pub use solver::{solve_encoded, solve_with_iterations};
...
mod wasm_interface {
    ...
    pub fn initialize() -> Result<JsValue, JsValue> {
        ...
        debug_log!(
            "[WASM] Dictionary loaded: {} words (filtered)",
            stats.word_count
        );
    ...
    pub fn encode_problem(size: usize) -> Result<JsValue, JsValue> {
        ...
        // Select words
        let suitable: Vec<String> = all_words
            .iter()
            .filter(|w| w.len() >= 3 && w.len() <= size)
            .cloned()
            .collect();

        let mut by_length: std::collections::HashMap<usize, Vec<String>> =
        ...
        let (num_vars, num_clauses) = encoder
            .encode(&words, size, target_quality)
        ...
        debug_log!(
            "[WASM] Encoded: {} vars, {} clauses in {}ms",
            num_vars,
            num_clauses,
            encoding_time
        );
        ...
        let mut state_lock = ENCODER_STATE
            .lock()
            .map_err(|e| JsValue::from_str(&format!("Lock error: {}", e)))?;
    ...
    pub fn solve_problem() -> Result<JsValue, JsValue> {
        ...
        let (encoder, words, size) = state_lock
            .take()
        ...
        let (placements, elapsed_ms) = solve_encoded(encoder).map_err(|e| JsValue::from_str(&e))?;

        debug_log!(
            "[WASM] Solved: {} placements in {}ms",
            placements.len(),
            elapsed_ms
        );
        ...
        debug_log!(
            "[WASM] Puzzle: density={:.1}%, {} words",
            puzzle.metadata.density * 100.0,
            puzzle.metadata.word_count
        );
    ...
    pub fn generate_crossword(size: usize, density: Option<usize>) -> Result<JsValue, JsValue> {
        ...
        debug_log!(
            "[WASM] generate_crossword: size={}, density={}%",
            size,
            target_density
        );
        ...
        let result = std::panic::catch_unwind(|| -> Result<CrosswordPuzzle, String> {
            ...
            let (placements, elapsed_ms, num_vars, num_clauses) =
                solver::solve_with_iterations(&words, size, target_density)?;

            debug_log!(
                "[WASM] Solved: {} placements in {}ms ({} vars, {} clauses)",
                placements.len(),
                elapsed_ms,
                num_vars,
                num_clauses
        ...
        match result {
            Ok(Ok(puzzle)) => serde_wasm_bindgen::to_value(&puzzle)
                .map_err(|e| JsValue::from_str(&format!("Serialization error: {}", e))),
            Ok(Err(e)) => Err(JsValue::from_str(&format!("Generation error: {}", e))),
            Err(_) => Err(JsValue::from_str("Panic during generation")),
        }
    }
}
