...
impl CrosswordEncoder {
    ...
    pub fn encode(
        ...
        debug_log!(
            "[ENCODER] Encoding {} words for {}x{} grid",
            words.len(),
            size,
            size
        );
            ...
            for y in 0..size {
                for x in 0..size {
                    ...
                    if y + word.len() <= size {
                        let pvar = self.new_var();
                        self.placement_vars
                            .insert((word.clone(), x, y, false), pvar);
                        all_placements.push(pvar);
        ...
        debug_log!(
            "[ENCODER] Created {} placement vars",
            self.placement_vars.len()
        );

        // Require at least one horizontal and one vertical word
        let horiz_placements: Vec<Var> = self
            .placement_vars
            .iter()
            .filter(|((_, _, _, h), _)| *h)
            .map(|(_, &v)| v)
        ...
        let vert_placements: Vec<Var> = self
            .placement_vars
            .iter()
            .filter(|((_, _, _, h), _)| !*h)
            .map(|(_, &v)| v)
        ...
        debug_log!(
            "[ENCODER] Added {} bidirectional grid clauses",
            _total_bidirectional_clauses
        );
        ...
        let filled_vars: Vec<Var> = is_filled
            .iter()
        ...
        if !filled_vars.is_empty() {
            debug_log!(
                "[ENCODER] Adding density constraint: at least {} of {} cells filled",
                min_filled_cells,
                filled_vars.len()
            );
        ...
        if min_quality > 0 {
            ...
            if !all_placements.is_empty() {
                ...
                debug_log!(
                    "[ENCODER] Quality constraint: at least {} placements",
                    min_words
                );
            }
        }
    ...
    fn add_sequence_constraint(
        ...
        let curr_chars: Vec<Var> = chars
            .iter()
            .filter_map(|&ch| self.grid_vars.get(&(x, y, ch)).copied())
            .collect();

        let (next_x, next_y) = if horizontal { (x + 1, y) } else { (x, y + 1) };
        let next_chars: Vec<Var> = chars
            .iter()
    ...
    fn at_most_one(&mut self, vars: &[Var]) {
        for i in 0..vars.len() {
            for j in (i + 1)..vars.len() {
                self.formula
                    .add_clause(&[vars[i].negative(), vars[j].negative()]);
            }
        }
    }
            ...
            for j in 0..=k.min(i) {
                ...
                if j == 0 {
                    ...
                    if let (Some(prev_j), Some(prev_jm1)) = (
                        aux[i - 1].get(j).and_then(|&o| o),
                        aux[i - 1].get(j - 1).and_then(|&o| o),
                    ) {
                        ...
                        self.formula
                            .add_clause(&[v.negative(), prev_j.positive(), x.positive()]);

                        // (prev_j AND NOT x) => v
                        self.formula
                            .add_clause(&[prev_j.negative(), x.positive(), v.positive()]);

                        // (prev_jm1 AND x) => v
                        self.formula
                            .add_clause(&[prev_jm1.negative(), x.negative(), v.positive()]);
                    ...
                    if let Some(prev) = aux[i - 1].get(j - 1).and_then(|&o| o) {
                        ...
                        self.formula
                            .add_clause(&[prev.negative(), x.negative(), v.positive()]);
                    }
                }
            }
        }
    ...
    pub fn extract_placements(&self, model: &[Lit]) -> Vec<Placement> {
        ...
        for ((x, y, ch), &var) in &self.grid_vars {
            if model_set.contains(&var.positive()) {
                ...
                if !covered {
                    debug_log!(
                        "[ENCODER] WARNING: Grid[{}][{}]='{}' not covered by any placement!",
                        x,
                        y,
                        ch
                    );
    ...
    fn add_connectivity_constraint(&mut self, size: usize, chars: &[char]) -> Vec<Vec<Var>> {
            ...
            for x in 0..size {
                ...
                if !cell_chars.is_empty() {
                    ...
                    for &cv in &cell_chars {
                        self.formula
                            .add_clause(&[cv.negative(), filled_var.positive()]);
                    }
                } else {
                    // Can't be filled
        ...
        for y in 0..size {
            ...
            for x in 0..size {
                ...
                self.formula
                    .add_clause(&[cc_start[y][x].negative(), cc_start_row[y].positive()]);

                // cc_start[y][x] => filled
                self.formula
                    .add_clause(&[cc_start[y][x].negative(), is_filled[y][x].positive()]);
                ...
                self.formula
                    .add_clause(&[cc_start[y][x].negative(), in_cc[y][x][0].positive()]);
                self.formula
                    .add_clause(&[cc_start[y][x].positive(), in_cc[y][x][0].negative()]);

                // Reachability propagation
                for i in 1..=max_dist.min(20) {
                    // Limit to 20 steps for performance
                    // in_cc[y][x][i] => filled AND (in_cc[y][x][i-1] OR neighbor_reaches_in_i-1)

                    self.formula
                        .add_clause(&[in_cc[y][x][i].negative(), is_filled[y][x].positive()]);

                    let mut reasons = vec![in_cc[y][x][i - 1].positive()];
                    if x > 0 {
                        reasons.push(in_cc[y][x - 1][i - 1].positive());
                    }
                    if x + 1 < size {
                        reasons.push(in_cc[y][x + 1][i - 1].positive());
                    }
                    if y > 0 {
                        reasons.push(in_cc[y - 1][x][i - 1].positive());
                    }
                    if y + 1 < size {
                        reasons.push(in_cc[y + 1][x][i - 1].positive());
                    }
                ...
                self.formula.add_clause(&[
                    is_filled[y][x].negative(),
                    in_cc[y][x][final_dist].positive(),
                ]);
            }
        }
