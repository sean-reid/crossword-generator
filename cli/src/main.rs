...
use clap::Parser;
use crossword_core::{solve_with_iterations, CrosswordPuzzle, Dictionary};
...
fn main() -> Result<()> {
    ...
    let dict = if let Some(ref allowlist_path) = args.allowlist {
        let allowlist_text =
            fs::read_to_string(allowlist_path).context("Failed to read allowlist file")?;
    ...
    println!(
        "\nGenerating {} puzzles of size {}x{} in parallel...",
        args.count, args.size, args.size
    );
    ...
    let latex_content = latex_gen
        .generate_document(&book)
        .context("Failed to generate LaTeX document")?;

    fs::write(&args.output, latex_content).context("Failed to write output file")?;
...
fn generate_crossword_with_density(
    dict: &Dictionary,
    size: usize,
    ...
    let (placements, elapsed_ms, _num_vars, _num_clauses) =
        solve_with_iterations(&words, size, density_percent)
            .map_err(|e| anyhow::anyhow!("Solver failed: {}", e))?;

    // Create puzzle
    let puzzle =
        CrosswordPuzzle::from_placements(&placements, size, |word| dict.get_clue(word), elapsed_ms);
...
fn compile_pdf(latex_path: &PathBuf) -> Result<()> {
    ...
    let check = Command::new("which").arg("pdflatex").output();
